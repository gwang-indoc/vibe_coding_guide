...
| [Claude Code: settings.json Permissions](ai-tools/claude-code-settings-json-permissions.md) | Baseline allow/deny template: Read/Glob/Grep/git-log/npm/node pre-approved; rm -rf/--force/curl/wget/env files/SSH/AWS credentials blocked; deny overrides allow | 2026-06-23 |
| [Claude Code: Session Monitor (claude_output)](ai-tools/claude-code-session-monitor.md) | Browser UI that tails ~/.claude/projects JSONL transcripts over WebSocket; live badge, multi-panel, Markdown export; read-only, localhost-only, ~1s latency; Express + ws + vanilla JS | 2026-06-23 |
